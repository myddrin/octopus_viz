...
def console_lines(
    ...
    *,
    ...
    start_period = min(
        conso_prices, key=attrgetter("consumption.interval_start")
    ).consumption.interval_start
    end_period = max(
        conso_prices, key=attrgetter("consumption.interval_end")
    ).consumption.interval_end

    ordered_consumption = sorted(
        conso_prices, key=attrgetter("consumption.consumption")
    )
    top_consumption = {
