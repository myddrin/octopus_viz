...
from octopus_viz.octopus_client.utils import handle_datetime, handle_timedelta, handle_date
...
class Tariff:
    ...
    def get_period_rates(self, period_start: datetime.datetime, period_end: datetime.datetime) -> list[PeriodRate]:
        ...
        for period in self.rates:
            if (
                period.interval_start <= start < period.interval_end
            ) and (
...
class Meter:
    ...
    def meters_from_config(cls, data: dict) -> list['Meter']:
        ...
        meter_configs = [
            ...
            for meter_kwargs in meters_data
        ]
...
class Config:
    ...
    def from_json(cls, filename: str) -> 'Config':
        ...
        logger.info(f'Loaded {len(meters)} configs and {n_tariffs} tariffs from "{filename}"')
